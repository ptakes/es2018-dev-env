...
describe('WebApiClient', () => {
  ...
  describe('constructor', () => {
    it('should set default server URL', () => {
      ...
      sut.serverUrl.href.should.equal('https://localhost/');
    });
    ...
    it('should set server URL', () => {
      const sut = new WebApiClient(new URL('https://server/path'));
      sut.serverUrl.href.should.equal('https://server/');
    });

    it('should set default API URL', () => {
      const sut = new WebApiClient();
      sut.apiUrl.href.should.equal('https://localhost/api');
    });

    it('should set API URL', () => {
      const sut = new WebApiClient(undefined, { apiUrl: '/api/v2/' });
      sut.apiUrl.href.should.equal('https://localhost/api/v2');
    });
  ...
  describe('authentication', () => {
    ...
    it('with anonymous call should not set Authorization header', async () => {
      ...
      await sut.getAsync('~/resources/1', {}, true).then(
        response => response,
        // @ts-ignore
        error => should.fail(error.message)
      );
    });
    ...
    it('with access token should set Authorization header with Bearer scheme', async () => {
      ...
      await sut.getAsync('~/resources/1').then(
        response => response,
        // @ts-ignore
        error => should.fail(error.message)
      );
  ...
  describe('GET', () => {
    ...
    it('not-existing resource should throw exception', async () => {
      ...
      const error = await sut.getAsync('~/resources/1').then(
        // @ts-ignore
  ...
  describe('POST', () => {
    ...
    it('with invalid payload should throw exception', async () => {
      ...
      const error = await sut.postAsync('~/resources').then(
        // @ts-ignore
        () => should.fail('No error thrown.'),
        error => error
      );
  ...
  describe('PUT', () => {
    ...
    it('with invalid payload should throw exception', async () => {
      ...
      const error = await sut.putAsync('~/resources/1').then(
        // @ts-ignore
        () => should.fail('No error thrown.'),
        error => error
      );
  ...
  describe('PATCH', () => {
    ...
    it('with invalid payload should throw exception', async () => {
      ...
      const error = await sut.patchAsync('~/resources/1').then(
        // @ts-ignore
        () => should.fail('No error thrown.'),
        error => error
      );
  ...
  describe('DELETE', () => {
    ...
    it('not-existing resource should throw exception', async () => {
      ...
      const error = await sut.deleteAsync('~/resources/1').then(
        // @ts-ignore
        () => should.fail('No error thrown.'),
        error => error
      );
